...
const ThoughtsForm = ({ newThoughtMessage, handleNewThought, onFormSubmit }) => {
  return (
    <form className="form-container" onSubmit={onFormSubmit}>
      ...
      <button className="submit-button" type="submit" aria-label="send happy thought" disabled={newThoughtMessage.length < 5 || newThoughtMessage.length > 140}>💛 Send happy thought 💛</button>
    </form>
  )
}
